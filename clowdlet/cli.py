...
def main() -> None:
    ...
    parser.add_argument("--db", help="generate the database Quadlet file if database is present (not generated by default)", action="store_true")
    ...
    for clowdapp in clowdapp_template.clowdapps:
        ...
        if clowdapp.database:
            print(f"  • {clowdapp.database.db_name} (Database)")
            print(f"      - PostgreSQL {clowdapp.database.db_version}")
    ...
    for clowdapp in clowdapp_template.clowdapps:
        clowdapp.write_quadlets(args.output, include_db=args.db)
